...
export default function TableSantander({ nome }) {
  ...
  const editTransactionSubmit = (id) => {
    firebaseDB.child(`/santander_fatura_atual/${id}`).set(
      {
        person,
        title,
        value,
  ...
  const handleDelete = (id) => {
    if (window.confirm("Deletar mesmo essa transação?")) {
      firebaseDB.child(`/santander_fatura_atual/${id}`).remove((err) => {
        if (err) {
          console.log(err);
        } else {
  ...
  return (
    <Box mt="6" w="100%" maxW="1480" borderRadius="8" bg="gray.800" p="8">
      <Flex mb="8" justify="space-between" align="center">
        ...
        >
          {nome} |
          <Text as="span" color="red.500" ml="2" fontSize="smaller">
            Santander
          </Text>
        </Flex>
      ...
      <Link href={`/resumeSantander/${nome}`} passHref>
        <Button
          p="4"
          mt="5"
          size="xs"
